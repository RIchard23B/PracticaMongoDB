...
const fileUpload = (req, res= response) =>{
    ...
    //Validar tipo
    const tiposValidos = ['categorias','usuarios']; //son los nombres de las carpetas donde registramos los archivos cargados
    if (!tiposValidos.includes(tipoColeccion)) {
        return res.status(400).json({
            ok: false,
            msg: 'No es un tipo valido (categoria, usuario)'
        });
    }
